...
public class SeptemberW3 {
    ...
    private boolean isValid(char[][] board, int row, int col) {
        return (isNotInRow(board, row) && isNotInCol(board, col)
                && isNotInBox(board, row - row % 3, col - col % 3));
    }

    private boolean isNotInBox(char[][] board, int startRow, int startCol) {
        ...
        for (int i = 0; i < 3; i++) {
            for (int j = 0; j < 3; j++) {
                ...
                if(curr != '.') {
                    if (!mSet.add(Integer.valueOf("" + curr))) {
    ...
    private boolean isNotInCol(char[][] board, int col) {
        Set<Integer> mSet = new HashSet<>();

        for (int k = 0; k < 9; k++) {
            if(board[k][col] != '.'){
                if(!mSet.add(Integer.valueOf(""+board[k][col]))){
                    System.out.println("Returning false from col " + board[k][col]);
                    return false;
                }
            }
        }
    ...
    private boolean isNotInRow(char[][] board, int row) {
        Set<Integer> mSet = new HashSet<>();

        for (int k = 0; k < 9; k++) {

            if(board[row][k] != '.'){
                if(!mSet.add(Integer.valueOf(""+board[row][k]))){
                    return false;
                }
            }
        }
